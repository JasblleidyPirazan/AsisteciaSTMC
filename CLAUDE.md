...
51. **Doble reporte también en las reposiciones grupales (deben coincidir):** una reposición ya no se cierra con el reporte de una sola persona. Ahora usa **la misma consolidación que una clase regular** (nota 31): `POST /makeups/:id/finalize` escribe el `ClassReport` del rol que llama (TEACHER→PROFESSOR, PHYSICAL_TRAINER/ADMIN→COORDINATOR, SUPERADMIN manda `reporterType`) y corre `consolidateSession()` → **PENDING** (falta uno: sin asistencia ni costo, `status` PROGRAMADA) · **MATCHED** (coinciden P/A/J por estudiante + quién dictó + asistente) → escribe el `AttendanceRecord`, `status=REALIZADA` y corre el motor de costos · **MISMATCH** → guarda el diff y entra a la cola de conflictos. Piezas compartidas movidas a `services/consolidation.js`: `ownerProfessorId(session)` (el "titular" es `group.professorId` en una clase regular y **`makeupProfessorId`** en una reposición, que no tiene grupo) y `resolveReporterType(role, requested)` (ADMIN→COORDINATOR; en clases regulares el ADMIN ni llega, `canReportGroup` lo bloquea antes). `GET /alerts/report-conflicts` incluye ahora `kind in [REGULAR, MAKEUP]` (el TEACHER filtra por su grupo **o** por ser el profesor/sustituto de la reposición) y devuelve `kind/title/makeupProfessor` para nombrarla sin grupo; `/admin/conflicts` la muestra y su botón lleva a `/makeups/:id/attendance`. El costEngine acepta `MATCHED` de MAKEUP como la parte del coordinador de la triple coincidencia. **Rescate de reposiciones legadas (`backfillLegacyReport`)**: las reportadas antes de este cambio tienen `AttendanceRecord` y costos pero ningún `ClassReport`; consolidarlas desde cero las mandaría a PENDING y **borraría asistencia y pagos ya liquidados**, así que al llegar el primer reporte nuevo se convierte la asistencia existente en el `ClassReport` del rol de quien la reportó (`reportedById` → su rol; sin usuario, COORDINATOR). Solo aplica a `kind=MAKEUP` ya REALIZADA y sin reportes. `PUT /makeups/:id` re-consolida cuando hay reportes (salvo si está CANCELADA: consolidar la descancelaría) y conserva el `calculateCosts` suelto para las legadas. Tests: `makeups.flow` (PENDING/MATCHED/MISMATCH), `makeupBackfill`, `consolidation` (`ownerProfessorId`/`resolveReporterType`).

50. **La reposición la cierran el profesor y el asistente, no el coordinador:** una reposición reportada por el coordinador quedaba `REALIZADA` y **desaparecía del home del profesor** (`PendingMakeups` pedía `status=PROGRAMADA`), así que él ya no podía poner su reporte aunque fuera el que dictó la clase. Ahora el **profesor** pide todas sus reposiciones (`GET /makeups` sin filtro de estado) y su home muestra las que le **falta reportar a él**: `status !== 'CANCELADA' && (status === 'PROGRAMADA' || reportedById !== su userId)`, con aviso "⚠️ Reportada por el coordinador · falta tu reporte". El **coordinador** sigue pidiendo solo las `PROGRAMADA` (si no, vería todas las que ya reportaron los profesores). `makeupInclude()` devuelve ahora `reportedBy {id, email, role}` y `assistantConfirmed {id, name}` (al ASSISTANT se le quita `reportedBy`), y `/admin/makeups` muestra por tarjeta los **dos pasos que faltan**: estado del reporte (⏳ sin reportar / ✅ reportada por el profesor / ⚠️ reportada por el coordinador, falta el profesor) y confirmación del asistente (⏳ falta que confirme / ✅ confirmó). El **asistente** ya veía y confirmaba reposiciones de cualquier estado menos CANCELADA (`AssistantDayView`), eso no cambió. Tests: `tests/integration/makeupsVisibility.test.js`.
