...
**Engine:** PostgreSQL (Prisma ORM v5, versioned migrations) · **Tables:** 29 · **Enums:** 16 · **Columns:** 270
...
| [`operating_expenses`](#operating_expenses) | `OperatingExpense` | Operating expenses of the academy that do NOT come from the class payroll (court rent, utilities, equipment, administrative payroll). |
| [`operating_expense_payments`](#operating_expense_payments) | `OperatingExpensePayment` | Marks ONE fortnight of an operating expense as paid. |
...
erDiagram
    ...
    operating_expenses ||--o{ operating_expense_payments : "expense_id"
...
*Prisma model:* `OperatingExpense` · *area:* Payroll · *columns:* 16

Operating expenses of the academy that do NOT come from the class payroll (court rent, utilities, equipment, administrative payroll). A FIJO row is a recurring definition: its amount is per fortnight and accrues in every fortnight overlapping [start_date, end_date] (a null end date means still in force). A VARIABLE row is a single expense charged to the fortnight in `period`. The "paid" mark lives in operating_expense_payments, one row per fortnight, because a fixed expense is paid fortnight by fortnight.

| Column | Type | Null | Key | Description |
|---|---|---|---|---|
| `id` | text | no | PK | Primary key. Collision-free id generated by the application (cuid). |
| `kind` | OperatingExpenseKind | no |  | FIJO (recurring per fortnight) or VARIABLE (one-off). |
| `category` | OperatingExpenseCategory | no |  | Accounting bucket. Default OTRO. |
| `concept` | text | no |  | What the money is spent on (free text, max 200 chars). |
| `amount` | numeric(12,2) | no |  | Amount in COP. For a FIJO expense this is the amount PER FORTNIGHT, never prorated by days. |
| `start_date` | date | yes |  | FIJO only: first day the expense is in force. Required for FIJO, null for VARIABLE. |
| `end_date` | date | yes |  | FIJO only: last day in force. Null = in force indefinitely. |
| `period` | text | yes |  | VARIABLE only: fortnight the expense is charged to ("YYYY-MM-1" / "YYYY-MM-2"), derived from expense_date. |
| `expense_date` | date | yes |  | VARIABLE only: date the expense happened. |
| `provider` | text | yes |  | Supplier / payee. |
| `note` | text | yes |  | Free note. |
| `active` | boolean | no |  | Soft delete. false = archived: stops accruing but keeps its history. Default true. |
| `created_by_id` | text | yes |  | Admin who registered the expense. |
| `created_by_name` | text | yes |  | Name of that admin, frozen at registration time. |
| `created_at` | timestamp | no |  | Row creation timestamp. |
| `updated_at` | timestamp | no |  | Last update, maintained by Prisma. |

**PK:** (id) · **Index:** (kind) · **Index:** (period)

**Relations**

- `payments` → `OperatingExpensePayment[]` — Fortnights of this expense already paid.

### `operating_expense_payments`

*Prisma model:* `OperatingExpensePayment` · *area:* Payroll · *columns:* 7

Marks ONE fortnight of an operating expense as paid. A fixed expense accumulates one row per fortnight paid; a variable one has at most a single row. Unique per (expense, fortnight).

| Column | Type | Null | Key | Description |
|---|---|---|---|---|
| `id` | text | no | PK | Primary key. Collision-free id generated by the application (cuid). |
| `expense_id` | text | no | FK → operating_expenses.id | Operating expense being paid. |
| `period` | text | no |  | Fortnight paid ("YYYY-MM-1" / "YYYY-MM-2"). |
| `paid_at` | timestamp | no |  | When it was marked paid. |
| `paid_by_id` | text | yes |  | Admin who marked it. |
| `paid_by_name` | text | yes |  | Name of that admin, frozen. |
| `created_at` | timestamp | no |  | Row creation timestamp. |

**PK:** (id) · **Unique:** (expense_id, period)

**Relations**

- `expense` → `OperatingExpense` — Expense being paid.
...
Nature of an operating expense of the academy (accounting module).

| Value | Meaning |
|---|---|
| `FIJO` | Recurring every fortnight: the amount is per fortnight and accrues in every fortnight touched by [start_date, end_date]. |
| `VARIABLE` | One-off expense, charged to the single fortnight in `period`. |

### `OperatingExpenseCategory`

Accounting bucket of an operating expense.

| Value | Meaning |
|---|---|
| `ARRIENDO` | Rent (courts, offices). |
| `SERVICIOS` | Utilities (water, power, internet). |
| `NOMINA_ADMINISTRATIVA` | Administrative payroll — staff who are not paid per class. |
| `MANTENIMIENTO` | Maintenance and repairs. |
| `IMPLEMENTOS` | Equipment and supplies (balls, nets, uniforms). |
| `TRANSPORTE` | Transport and travel. |
| `MARKETING` | Advertising and promotion. |
| `IMPUESTOS_SEGUROS` | Taxes, fees and insurance. |
| `OTRO` | Anything else. Default. |
