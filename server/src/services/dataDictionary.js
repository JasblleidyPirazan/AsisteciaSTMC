...
const ENUMS = [
  ...
  {
    name: 'OperatingExpenseKind',
    description: 'Nature of an operating expense of the academy (accounting module).',
    values: [
  ...
  },
  {
    ...
    description: 'Accounting bucket of an operating expense.',
    values: [
...
const TABLES = [
  ...
  },
  {
    ...
    columns: [
      pk(),
      col('kind', 'kind', 'OperatingExpenseKind', false, 'FIJO (recurring per fortnight) or VARIABLE (one-off).'),
      col('category', 'category', 'OperatingExpenseCategory', false, 'Accounting bucket. Default OTRO.'),
      col('concept', 'concept', 'text', false, 'What the money is spent on (free text, max 200 chars).'),
      col('amount', 'amount', 'numeric(12,2)', false, 'Amount in COP. For a FIJO expense this is the amount PER FORTNIGHT, never prorated by days.'),
      col('start_date', 'startDate', 'date', true, 'FIJO only: first day the expense is in force. Required for FIJO, null for VARIABLE.'),
      col('end_date', 'endDate', 'date', true, 'FIJO only: last day in force. Null = in force indefinitely.'),
      col('period', 'period', 'text', true, 'VARIABLE only: fortnight the expense is charged to ("YYYY-MM-1" / "YYYY-MM-2"), derived from expense_date.'),
      col('expense_date', 'expenseDate', 'date', true, 'VARIABLE only: date the expense happened.'),
      col('provider', 'provider', 'text', true, 'Supplier / payee.'),
      col('note', 'note', 'text', true, 'Free note.'),
      col('active', 'active', 'boolean', false, 'Soft delete. false = archived: stops accruing but keeps its history. Default true.'),
      col('created_by_id', 'createdById', 'text', true, 'Admin who registered the expense.'),
      col('created_by_name', 'createdByName', 'text', true, 'Name of that admin, frozen at registration time.'),
      createdAt(),
      col('updated_at', 'updatedAt', 'timestamp', false, 'Last update, maintained by Prisma.'),
    ],
    keys: { primary: ['id'], indexes: [['kind'], ['period']] },
    relations: [['payments', 'OperatingExpensePayment[]', 'Fortnights of this expense already paid.']],
  },
  {
    model: 'OperatingExpensePayment',
    table: 'operating_expense_payments',
    delegate: 'operatingExpensePayment',
    group: 'Payroll',
    description:
    ...
    columns: [
      pk(),
      col('expense_id', 'expenseId', 'text', false, 'Operating expense being paid.', { key: 'FK → operating_expenses.id' }),
      col('period', 'period', 'text', false, 'Fortnight paid ("YYYY-MM-1" / "YYYY-MM-2").'),
      col('paid_at', 'paidAt', 'timestamp', false, 'When it was marked paid.'),
      col('paid_by_id', 'paidById', 'text', true, 'Admin who marked it.'),
      col('paid_by_name', 'paidByName', 'text', true, 'Name of that admin, frozen.'),
      createdAt(),
    ],
    keys: { primary: ['id'], unique: [['expense_id', 'period']] },
    relations: [['expense', 'OperatingExpense', 'Expense being paid.']],
  },
  {
