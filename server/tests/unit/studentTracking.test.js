...
import { buildTrackingRows, countRecords, countRain, consumedTotal, progressPct, isMakeupRecord }
...
describe('countRecords — conteo por estudiante', () => {
  it('separa asistencias, faltas, justificadas, N/A y reposiciones', () => {
    ...
    expect(c).toEqual({ present: 2, absent: 1, justified: 1, na: 1, makeup: 2, rain: 0 });
  });
...
describe('buildTrackingRows — filas de la vista', () => {
  const students = [{
    ...
    enrollments: [
      { enrollmentType: 'PRIMARY', group: { id: 'g1', code: 'MJ1522', ballLevel: 'Verde', professor: { name: 'Ana' } } },
      { enrollmentType: 'SECONDARY', group: { id: 'g2', code: 'S541', ballLevel: 'Verde', professor: { name: 'Beto' } } },
    ],
  }];
  const records = [
  ...
  const [row] = buildTrackingRows({ students, records, rainDatesByGroup: { g1: [new Date('2026-03-09')] } });
  ...
  it('trae los conteos y la lluvia', () => {
    expect(row).toMatchObject({ present: 1, absent: 1, makeup: 1, justified: 0, na: 0, rain: 1 });
  });
  it('un estudiante sin registros queda en ceros', () => {
    ...
    expect(solo).toMatchObject({ present: 0, absent: 0, makeup: 0, rain: 0, acquired: 8, groupCode: null });
  });
});
